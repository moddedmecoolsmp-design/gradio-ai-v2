"""
LoRA (Low-Rank Adaptation) support for Z-Image on CUDA.

This module provides a simplified LoRA implementation that works with
quantized models on NVIDIA GPUs (CUDA). LoRA weights are applied as
...
LINEAR_MODULES = ['Linear', 'LoRACompatibleLinear', 'QLinear', 'SDNQLinear']
...
class LoRAModule(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        # Ensure x matches the original module's weight dtype when weights are floating-point.
        # Avoid casting to quantized (uint8) weights, which breaks SDNQ linear layers.
        org_module = self.org_module[0]
        if hasattr(org_module, "weight") and hasattr(org_module.weight, "dtype"):
            if torch.is_floating_point(org_module.weight) and x.dtype != org_module.weight.dtype:
...
class LoRANetwork(nn.Module):
    ...
    # Target modules for Z-Image and Flux transformers
    TARGET_MODULES = ["ZImageTransformer2DModel", "FluxTransformer2DModel", "Flux2Transformer2DModel"]
    ...
    def _convert_weight_keys(self, weights: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for key, value in weights.items():
            ...
            new_key = new_key.replace('_lora_A', '.lora_down')
            new_key = new_key.replace('_lora_B', '.lora_up')

            # Convert dots to underscores for the module part to match our add_module naming
            # transformer.name.lora_down.weight -> transformer_name.lora_down.weight
            parts = new_key.split('.')
            if len(parts) >= 3:
                # Rejoin the middle parts with underscores
                # parts[-2] is lora_down/up, parts[-1] is weight/bias
                main_path = "_".join(parts[:-2])
                new_key = f"{main_path}.{parts[-2]}.{parts[-1]}"
...
def load_lora_for_pipeline(
    ...
    Args:
        ...
        dtype: Dtype for LoRA weights (defaults to float16 for CUDA efficiency)
    ...
    # Use float16 for CUDA efficiency, bfloat16 also works on 30/40 series GPUs
